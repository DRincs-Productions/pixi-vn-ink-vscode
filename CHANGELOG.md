...
- **pixi-vn engine only**: an unknown hashtag command that's a very close match for a registered Custom Hashtag Command's Zod schema — a distraction typo, like `# shwo image bg` for `# show image bg`, or `# jump` missing its required target — now has the specific mismatch appended to the existing "unknown hashtag command" warning: the exact wrong token and what it should have been (e.g. `must be "show"`), or the missing/extra argument, instead of just the generic "no registered handler matched this command" message. This is powered by a new `InkCompiler.getLikelyUnknownHashtagCommandSchemaIssues` utility added to `@drincs/pixi-vn-ink` itself: it scores an unknown command's tokens against every registered `"zod"`-validated handler (weighted toward the literal/enum tokens that actually identify a command, e.g. `show`/`video`, over free-form arguments), and only once a candidate scores very high does it re-validate against that one schema with Ajv (specifically an `Ajv2020` instance — the plain draft-07 one used elsewhere silently ignores the `prefixItems` keyword zod's tuple schemas rely on) to find the concrete issue to report. Deliberately kept out of `vitePluginInk` (heavier than a build-time check needs) but exposed as a standalone, reusable utility for any project that wants it. Only takes effect once the installed `@drincs/pixi-vn-ink` version actually ships this method — see the related entry below for what happens until then.
- **pixi-vn engine only**: a hashtag command registered with `HashtagCommands.add(..., { deprecated: true })` is now struck through wherever it's used in a `.ink` file, exactly like the built-in JavaScript/TypeScript extension marks a deprecated API — hovering the strikethrough now also explains that the command is deprecated (including its own description, when the handler has one). Requires `@drincs/pixi-vn-ink` 1.1.8 or later: earlier versions' Vite dev-server serialized a registered handler's info without its `deprecated` flag at all, so this extension had no way to learn it regardless of what a project's own handler registration set.
...
- **pixi-vn engine only**: hovering `_input_value_` now explains that it contains the result from an input prompt, with a clickable link to the [Pixi'VN input documentation](https://pixi-vn.com/ink/input). Under the Inky engine it remains an ordinary variable name with no special popup.
